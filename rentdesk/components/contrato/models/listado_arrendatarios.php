...
$orderby = " ORDER BY td.descripcion asc";
switch ($orden) {
	case 0:
...
$query= "SELECT p.token,p.id_persona,td.descripcion tipo_documento,
		...
		CONCAT(p.nombre,' ',p.apellido_pat,' ',p.apellido_mat) arrendatario
		FROM arpis.persona p,
			 arpis.tipo_documento td,
			 arpis.estado_persona ep
		WHERE td.id_tipo_documento = p.id_tipo_documento
...
$asignar = "";
$asignar = "<a href='javascript: agregarArrendatario(\\\"$result->token\\\",\\\"$result->arrendatario\\\");'><i class='fas fa-plus-square'></i></a>"; 
