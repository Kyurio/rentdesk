...
function cargarArriendoOtrosPagosList() {
  var idFicha = $("#id_ficha").val();

  $("#cobro-arriendo-otros-pagos-table").DataTable({
    dom: 'B<"clear">lfrtip',
    destroy: true,
    targets: "no-sort",
    bSort: false,
    ...
    ajax: {
      url: "components/cobro/models/cobro_arriendo_otros_pagos_list_procesa.php",
      type: "POST",
    },
    language: {
  ...
  $("#cobro-arriendo-otros-pagos-table").on("init.dt", function () {
    console.log("DataTables se ha inicializado correctamente en #cobro-arriendo-otros-pagos-table");
  });
}
...
function validaArchivo(e) {
  ...
  alert("Please select a PDF file COBRO: ", e);
