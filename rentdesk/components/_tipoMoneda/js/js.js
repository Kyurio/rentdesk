...
var formData = new FormData(document.getElementById("formulario"));
   ...
   $.ajax({
                url: "components/tipoMoneda/models/insert_update.php",
                type: "post",
                dataType: "html",
                data: formData,
                cache: false,
             ...
             .done(function(res){
				...
				if (resultado == 'OK'){
					...
					document.location.href="index.php?component=tipoMoneda&view=tipoMoneda&token="+token;
					return false;
				}else{
...
$(document).ready(function() { 
  ...
  $('#tabla').DataTable( {
	   "order": [[ 3, "asc" ]],
        ...
        "ajax": {
			"url":"components/tipoMoneda/models/tipoMoneda_list_procesa.php",
		"type": "POST"},
		"language": {
...
//************************************************************************
function deleteTipo_moneda(token){
	...
	onSubmit: function (result) {
		if(result) {
				  $.ajax({
					  type: 'POST',
					  url: "components/tipoMoneda/models/delete.php",
					  data:  "token="+token,
