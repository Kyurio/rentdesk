...
var formData = new FormData(document.getElementById("formulario"));
   ...
   $.ajax({
                url: "components/tipoProducto/models/insert_update.php",
                type: "post",
                dataType: "html",
                data: formData,
                cache: false,
             ...
             .done(function(res){
				...
				if (resultado == 'OK'){
					...
					document.location.href="index.php?component=tipoProducto&view=tipoProducto&token="+token;
					return false;
				}else{
...
$(document).ready(function() { 
  ...
  $('#tabla').DataTable( {
	   "order": [[ 2, "asc" ]],
        ...
        "ajax": {
			"url":"components/tipoProducto/models/tipoProducto_list_procesa.php",
		"type": "POST"},
		"language": {
...
//************************************************************************
function deleteTipo_producto(token){
	...
	onSubmit: function (result) {
		if(result) {
				  $.ajax({
					  type: 'POST',
					  url: "components/tipoProducto/models/delete.php",
					  data:  "token="+token,
