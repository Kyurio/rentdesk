...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($token)) {
        ...
        // Actualizar el registro en lugar de insertar uno nuevo
        $update = "UPDATE propiedades.ficha_arriendo_cta_cte_movimientos SET
            fecha_movimiento = '$fecha',
            hora_movimiento = '$time',
            monto = $monto,
            razon = '$razon',
            mes_imputado = '$mes_imputado'
            WHERE id = $id";

        $dataCab = array("consulta" => $update);
