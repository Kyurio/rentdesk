...
$arrendatarios = "";
$fecha = date("Y-m-d");
$carpeta = "upload\arriendo_cuenta_corriente";
...
foreach ($_FILES as $clave => $archivo) {
    if (strpos($clave, 'archivo_cuenta_corriente_') !== false) {
        // Aquí puedes acceder a la información del archivo
        $nombre_archivo = $archivo["name"];
        $tipo_archivo = $archivo["type"];
        $tamano_archivo = $archivo["size"];
        ...
        $indice = substr($clave, strlen('archivo_cuenta_corriente_'));
             ...
             //   var_dump("Archivo", $nombre_sin_extension, "Fecha de Documento", $fechaDocumento);
				...
				//move_uploaded_file($_FILES["archivo"]["tmp_name"], "../../../upload/arriendo_cuenta_corriente/" . $doc_ima_fisico.".".$tipo_archivo);
                    ...
                    (id, id_usuario, fecha_subida, ruta, componente, archivo, titulo, fecha_vencimiento, id_ficha_arriendo, estado,extension,token_agrupador,nombre_archivo)
                     VALUES(nextval('propiedades.propiedad_archivos_id_seq'::regclass),$objUsuarioId->id, '$fecha','$carpeta' ,'arriendo_cuenta_corriente', '$doc_ima_fisico' , '$documentoTitulo', '$fechaDocumento', $objArriendoId->id,true,'$extension','$token_agrupador','$nombre_sin_extension') ";
...
//	var_dump($query);
	...
	$queryCabecera= " INSERT INTO propiedades.propiedad_archivos
                    (id, id_usuario, fecha_subida, ruta, componente, archivo, titulo, fecha_vencimiento, id_ficha_arriendo, estado,extension)
                     VALUES(nextval('propiedades.propiedad_archivos_id_seq'::regclass),$objUsuarioId->id, '$fecha','$carpeta' ,'arriendo_cuenta_corriente', '$doc_ima_fisico' , '$documentoTitulo', '$documentoFecha', $objArriendoId->id,true,'$extension') ";
