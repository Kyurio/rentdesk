...
$query = " select * from propiedades.cuenta_rol_componentes where id_rol = $idRol ";
$cant_rows = $num_reg;
$num_pagina = round($inicio / $cant_rows) + 1;
$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
$json_rol_item = json_decode($resultado);
*/
...
foreach ($json as $item) {
	...
	$query = " select count(*) as cantidad  from propiedades.cuenta_rol_componentes where id_rol = $idRol and id_componente_item = $item->id ";
	$cant_rows = $num_reg;
	$num_pagina = round($inicio / $cant_rows) + 1;
	$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
	$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
	//var_dump($query);
	$json_rol_item = json_decode($resultado)[0];
	
	if ($json_rol_item->cantidad > 0 ){
		$rol_propiedad_editar = $rol_propiedad_editar . "<option  value='$item->id' selected >$item->label</option>";

	}else{
		$rol_propiedad_editar = $rol_propiedad_editar . "<option  value='$item->id'  >$item->label</option>";
...
foreach ($json as $item) {
	...
	if ($json_rol_item->cantidad > 0 ){
		$rol_administracion_editar = $rol_administracion_editar . "<option  value='$item->id' selected >$item->label</option>";
...
foreach ($json as $item) {
	...
	if ($json_rol_item->cantidad > 0 ){
		$rol_cliente_editar = $rol_cliente_editar . "<option  value='$item->id' selected >$item->label</option>";
...
foreach ($json as $item) {
	...
	if ($json_rol_item->cantidad > 0 ){
		$rol_arriendo_editar = $rol_arriendo_editar . "<option  value='$item->id' selected >$item->label</option>";
...
foreach ($json as $item) {
	...
	if ($json_rol_item->cantidad > 0 ){
		$rol_acciones_editar = $rol_acciones_editar . "<option  value='$item->id' selected >$item->label</option>";
...
foreach ($json as $item) {
	...
	if ($json_rol_item->cantidad > 0 ){
		$rol_facturacion_editar = $rol_facturacion_editar . "<option  value='$item->id' selected >$item->label</option>";
...
foreach ($json as $item) {
	...
	if ($json_rol_item->cantidad > 0 ){
		$rol_reporte_editar = $rol_reporte_editar . "<option  value='$item->id' selected >$item->label</option>";
...
//echo ",xxx,OK,xxx,$rol_propiedad_editar,xxx,$rol_acciones_editar,xxx,$rol_administracion_editar,xxx,$rol_arriendo_editar,xxx,$rol_facturacion_editar,xxx,$rol_cliente_editar,xxx,$rol_reporte_editar,xxx,";
...
$query = " select * from propiedades.menu_componentes_items where id_grupo = 15 ";
$cant_rows = $num_reg;
$num_pagina = round($inicio / $cant_rows) + 1;
$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
...
$rol_archivo = "<option value=''>Seleccione</option>";
...
foreach ($json as $item) {
	...
	if ($json_rol_item->cantidad > 0 ){
		$rol_archivo = $rol_archivo . "<option  value='$item->id' selected >$item->label</option>";

	}else{
		$rol_archivo = $rol_archivo . "<option  value='$item->id'  >$item->label</option>";
...
echo ",xxx,OK,xxx,$rol_propiedad_editar,xxx,$rol_acciones_editar,xxx,$rol_administracion_editar,xxx,$rol_arriendo_editar,xxx,$rol_facturacion_editar,xxx,$rol_cliente_editar,xxx,$rol_reporte_editar,xxx,$rol_archivo,xxx,";
