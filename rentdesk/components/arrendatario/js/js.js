  ...
  $('#descargarExcelArrendatario').on('click', function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        // response es un array de objetos JSON. Ejemplo:
        // [{id_ficha_arriendo:..., id_persona:..., dni:..., nombre_completo:...}, ...]

        // Convertir los datos JSON a una hoja (worksheet) de Excel
        // Primero, convertir el array de objetos a un formato entendible por SheetJS.
...
function loadArrendatario_List() {
  $(document).ready(function () {
    ...
    var ajaxUrl = "components/arrendatario/models/arrendatario_list_procesa.php?" +
        ...
        buttons: [
          {
            ...
            customize: function (xlsx) {
              ...
              $(sheet).find('row c[r^="' + String.fromCharCode(65 + columnIndexToRemove) + '"]').remove(); // Elimina todas las celdas de esa columna

              // Eliminar también la celda del encabezado de la columna de acciones
              $(sheet).find('row:first c[r^="' + String.fromCharCode(65 + columnIndexToRemove) + '"]').remove();
              ...
              $(sheet).find('row').each(function (rowIdx) {
                if (rowIdx > 0) { // Salta la fila de encabezado
                  const cell = $(this).find(`c[r^="${String.fromCharCode(65 + precioColumnIndex)}"]`);
                  if (cell.length) {
...
function buscarArrendatarioAutocompleteGenerica(valor, tipo) {
  ...
  if (caracteres >= 3) {
    $.ajax({
      type: "POST",
      url:
        "components/arrendatario/models/buscar_arrendatario_autocomplete_generica.php",
