...
if (isset($token) && $token !== '') {
	...
	$query = "SELECT * FROM propiedades.vis_arrendatarios where token_arrendatario = '$token'";
	$cant_rows = $num_reg;
	$num_pagina = round($inicio / $cant_rows) + 1;
	$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
	$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);

	$resultArrendatario = json_decode($resultado)[0];


	// //var_dump("PERSONA POR TOKEN: ", $result);
}

if (isset($resultArrendatario->token_persona)) {

	/*BUSQUEDA PERSONA POR TOKEN EN PROPIETARIO */
	$queryParams = array(
		'token_subsidiaria' => $current_subsidiaria->token,
		"token_persona" => $resultArrendatario->token_persona
	);
...
foreach ($json as $item) {
	...
	// //var_dump("BANCO JSON: ", @$item->id == @$result->arrendatario->cuentasBancarias[0]->banco->id);

	if (@$result->arrendatario->cuentasBancarias[0]->banco->id == @$item->id) {
...
foreach ($json as $item) {
	...
	if (@$result->arrendatario->cuentasBancarias[0]->tipoCuenta->id == @$item->id) {
...
foreach ($json as $item) {
	...
	if (@$resultArrendatario->token_persona == @$item->token) {
...
$opcion_persona = "<select id='persona' name='persona' class='form-control' required onchange='onChangePersona();' $readOnlyAttr>
$opcion_persona
</select>";
...
$num_reg = 500;
$inicio = 0;
$queryCuetnas="  select tb.nombre as nombre_banco,tcb.nombre as tipo_cuenta, 
acb.rut_titular, acb.nombre_titular,acb.correo_electronico, 
acb.nombre_titular, acb.numero, acb.id, tb.id as id_banco, 
tcb.id as id_tipo_cuenta 
from propiedades.persona_arrendatario pa 
left join propiedades.arrendatario_ctas_bancarias acb on pa.id_persona =acb.id_arrendatario  
inner join propiedades.tp_banco tb on tb.id = acb.id_banco 
inner join propiedades.tp_cta_bancaria tcb on tcb.id = acb.id_tipo_cta_bancaria 
     where pa.token = '$token' and acb.habilitado = true ";
$num_pagina = round($inicio / $cant_rows) + 1;
