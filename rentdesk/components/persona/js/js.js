...
$(document).ready(function () {
  ...
  texto = 'Por favor completa los datos del cliente para asignarlo a la propiedad';
...
function generarExcel(urlbase) {
  ...
  var ajaxUrl = "components/persona/models/listado_personas_procesa_excel.php?" +
    "dniCliente=" + encodeURIComponent(dniCliente) +
    "&propietario=" + encodeURIComponent(propietario) +
    "&arrendatario=" + encodeURIComponent(arrendatario) +
    "&codeudor=" + encodeURIComponent(codeudor);
  $.ajax({
    type: 'GET',
    url: ajaxUrl,
    ...
    $.ajax({
      ...
      .done(function (res) {
        ...
        if (rutcapturado) {
          ...
          window.location.href = "index.php?component=persona&view=persona_list";
        } else {
          ...
          if (resultado == "OK") {
            ...
            Swal.fire({
              ...
              text:
                "Se le redireccionara para que pueda ver y modificar al cliente ingresado",
              icon: "info",
              showConfirmButton: true,
              allowOutsideClick: false, // Evita que el usuario cierre haciendo clic fuera del cuadro
              willClose: () => {
...
function loadCliente_List() {
  $(document).ready(function () {
      ...
      var ajaxUrl = "components/persona/models/listado_personas_procesa.php?" +
          "dniCliente=" + encodeURIComponent(dniCliente) +
          "&propietario=" + encodeURIComponent(propietario) +
          "&arrendatario=" + encodeURIComponent(arrendatario) +
          "&codeudor=" + encodeURIComponent(codeudor);
              ...
              buttons: [
                  {
                      ...
                      customize: function (xlsx) {
                          ...
                          $('row c[r^="' + String.fromCharCode(65 + columnIndexToRemove) + '"]').remove(); // Elimina todas las celdas de esa columna
