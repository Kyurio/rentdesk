...
function cargarRetencionesList() {
	let idFicha = $('#id_ficha').val();

	// Verificar ID desde localStorage si no está en el DOM
	if (!idFicha) {
		console.warn(
	...
	$.ajax({
		...
		success: function (response) {
			...
			const columns = [
				...
				{ data: 'estado' },
				{
					data: null,
					render: function (data, type, row) {
						// Mostrar botón de eliminación solo si el estado permite la acción
						if (
		...
		error: function (jqXHR, textStatus, errorThrown) {
			...
			Swal.fire({
				icon: 'error',
				title: 'Error al cargar las retenciones',
				text: 'Hubo un problema al intentar obtener los datos. Por favor, inténtelo de nuevo más tarde.',
			});
		},
	});
}
...
document.addEventListener('DOMContentLoaded', function () {
			...
			fetch('components/propiedad/models/insert_retenciones.php', {
				...
				.then((result) => {
					if (result.status === 'success') {
						...
						// Cerrar el modal utilizando la API de Bootstrap
	...
	function limpiarCampos() {
		...
		document.getElementById('tipo_retencion').value = '1';
		document.getElementById('razonRetencion').value = ''; // Limpia este campo
		fechasContainer.style.display = 'none';
		montoContainer.style.display = 'flex';
		razonContainer.style.display = 'flex';
	}
