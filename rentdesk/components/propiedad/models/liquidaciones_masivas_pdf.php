...
// 1. Recibir el valor del array por POST
$idPropiedades = $_POST['idPropiedades'];

// 2. Convertir el string recibido a un array
$idPropiedadesArray = json_decode($idPropiedades, true);

// 3. Recorrer el array
foreach ($idPropiedadesArray as $id) {
...
(id_ficha_propiedad, fecha_liquidacion, id_ficha_arriendo, url_liquidacion)
values( '$ficha_tecnica_propiedad', '$fecha_liquidacion','$id_ficha_arriendo', '$ruta_info' )";
...
foreach ($objArrendatario as $item) {
    ...
    $i++;
    $arrenatario = mb_convert_encoding($item->nombre_1 . " " . $item->nombre_2 . " " . $item->nombre_3 . " ", "ISO-8859-1", "UTF-8");
...
foreach ($objMovimientos as $mov) {
    ...
    if ($mov->haber == 0) {
        ...
        $cargos = number_format($mov->haber, 0, '', '.');
    ...
    if ($mov->debe == 0) {
        ...
        $abonos = number_format($mov->debe, 0, '', '.');
    ...
    $pdf->SetFillColor(255, 255, 255);
    $pdf->Row(array(date("d-m-Y", strtotime($mov->fecha_movimiento)), $mov->razon, $cargos, $abonos), 6.5, 'C', true);
...
if ($total_comision_adm != 0 && $moneda_comision_adm == 1 ) {
    ...
    $pdf->Cell(38, 6, "", 1, 0, 'L', true);
    $pdf->Cell(58, 6, "Comision Administracion $" . $monto_administracion_comision . " CLP", 1, 0, 'L', true);
    ...
    $pdf->Cell(38, 6, "", 1, 0, 'L', true);
    $pdf->Cell(58, 6, "Comision Administracion " . $monto_administracion_comision . " UF", 1, 0, 'L', true);
    $pdf->Cell(58, 6, "" . number_format($comision_administracion_fija, 0, '', '.'), 1, 0, 'R', true);
    $pdf->Cell(38, 6, "", 1, 1, 'R', true);
...
if($moneda_comision_arr == 3 ){
    ...
    $pdf->Cell(38, 6, "", 1, 0, 'L', true);
    $pdf->Cell(58, 6, "" . $adm_comision_primer_liquidacion_uf ." UF  Comision Arriendo", 1, 0, 'L', true);
...
if($moneda_comision_arr == 2)	{
    ...
    $pdf->Cell(38, 6, "", 1, 0, 'L', true);
    $pdf->Cell(58, 6, "$" . $monto_arriendo_comision . " CLP Comision Arriendo", 1, 0, 'L', true);
    $pdf->Cell(58, 6, "" . number_format($valor_comision_arriendo, 0, '', '.'), 1, 0, 'R', true);
    $pdf->Cell(38, 6, "", 1, 1, 'R', true);
}
...
$pdf->Ln(9);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 10, mb_convert_encoding("Garantia", "ISO-8859-1", "UTF-8"), 0, 0, 'L');
$pdf->Ln(8);
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetFillColor(217, 237, 247);
$pdf->SetFont('Arial', 'B', 8.5);
$pdf->Cell(116, 6, mb_convert_encoding("Garantia", "ISO-8859-1", "UTF-8"), 1, 0, 'L', true);
$pdf->Cell(38, 6, mb_convert_encoding("Fecha Cuota", "ISO-8859-1", "UTF-8"), 1,  0, 'L', true);
$pdf->Cell(38, 6, mb_convert_encoding("Valor", "ISO-8859-1", "UTF-8"), 1, 1, 'R', true);
...
$pdf->SetFillColor(255, 255, 255);
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(116, 6, mb_convert_encoding("Cuota $numeroCuota de $num_cuotas_garantia", "ISO-8859-1", "UTF-8"), 1, 0, 'L', true);
$pdf->Cell(38, 6, mb_convert_encoding("$nombreMes del $anioGarantia", "ISO-8859-1", "UTF-8"), 1,  0, 'L', true);
$pdf->Cell(38, 6, mb_convert_encoding("$".number_format($montoGarantia, 0, '', '.')."", "ISO-8859-1", "UTF-8"), 1, 1, 'R', true);
$pdf->SetFillColor(255, 255, 255);
}
}
...
foreach ($arrayDeIds as $idMov) {
    ...
    $resultadoActualizacionLiq = $services->sendPostDirecto($url_services . '/util/dml', $dataCab);
}

$pdf->Output('F', $ruta_guardado);
echo '<script>window.location.href = "../../../index.php?component=propiedad&view=liquidacion_historico";
</script>';
