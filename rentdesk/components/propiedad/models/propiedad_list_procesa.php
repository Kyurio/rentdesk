...
if ($filtro_region != "") {
	...
	$cant_rows = $num_reg;
	$num_pagina = round($inicio / $cant_rows) + 1;
	$data = array("consulta" => $queryRegion, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
	$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
	$json_region = json_decode($resultado)[0];
}

if ($filtro_comuna != "") {
	$queryComuna = " select * from propiedades.tp_comuna where id = $filtro_comuna ";
	$cant_rows = $num_reg;
	$num_pagina = round($inicio / $cant_rows) + 1;
	$data = array("consulta" => $queryComuna, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
	$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
	$json_comuna = json_decode($resultado)[0];
}

if ($filtro_propietario == "") {
	$query = "SELECT cs.nombre as nombre_sucursal ,vp.* ,cu.*,  concat(
            vp.direccion, 
            ' #', vp.numero, 
            CASE 
    ...
    FROM propiedades.vis_propiedades vp 
    JOIN propiedades.cuenta_sucursal cs ON cs.id = vp.id_sucursal 
    JOIN propiedades.cuenta_usuario cu ON cu.id = vp.id_ejecutivo 
    WHERE vp.habilitado = true 
    AND vp.rol_principal = true
    AND cs.id = vp.id_sucursal 
	...
	$query = "SELECT DISTINCT  vp.*, CONCAT(
                vp.direccion, 
                ' #', vp.numero, 
                CASE 
                    WHEN vp.numero_depto IS NOT NULL AND vp.numero_depto <> '' THEN CONCAT(' Dpto ', vp.numero_depto) 
    ...
    FROM propiedades.vis_propiedades vp
    INNER JOIN propiedades.cuenta_sucursal cs ON cs.id = vp.id_sucursal
    INNER JOIN propiedades.cuenta_usuario cu ON cu.id = vp.id_ejecutivo
    INNER JOIN propiedades.propiedad_copropietarios pc ON vp.id_propiedad = pc.id_propiedad
    INNER JOIN propiedades.vis_propietarios vp2 ON pc.id_propietario = vp2.id
    WHERE vp.habilitado = true 
    AND vp.rol_principal = true 
...
if (isset($filtro_propietario) && $filtro_propietario !== "") {
	...
	$whereConditions[] = " UPPER(vp2.nombre_1) LIKE UPPER('%$filtro_propietario%')
							OR UPPER(vp2.nombre_2) LIKE UPPER('%$filtro_propietario%')
							OR UPPER(vp2.dni) LIKE UPPER('%$filtro_propietario%')
...
if (!empty($whereConditions)) {
	...
	$query .= " order by estado_propiedad, id_propiedad desc  ";
} else {
	$query .= " order by estado_propiedad, id_propiedad desc ";
}
...
if ($json !== null) {
	if ($filtro_propietario == "") {
		...
		foreach ($json as $obj) {
			...
			// $precio = '$' . number_format($precio, 0, ',', '.'); // Formato: $1.234.567

			if ($precio[0]['id_moneda_precio'] == 2) { // es peso
				$montoFomateado = '$ ' . number_format($precio[0]['precio'], 0, '.', '.');
			} else if ($precio[0]['id_moneda_precio'] == 1) {
				$montoFomateado = '% ' . $precio[0]['precio'];
			} else if ($precio[0]['id_moneda_precio'] == 3) {
				$montoFomateado = 'UF ' . $precio[0]['precio'];
			} else {
				$montoFomateado = $precio[0]['precio'];
			}
			...
			$transformedObj = [
				...
				'direccion' => validateNull($obj->direccion_concat ?? null),
			...
			if (@$json_existe_arriendo->existe != "SI") {
				...
				$botones = $botones . "<button type='button' onclick='avisoPropiedad()' class='btn btn-secondary m-0' style='padding: .5rem;' title='Eliminar'><i class='fa-regular fa-trash-can' style='font-size: .75rem;'></i> </button>";
			}

			if ($obj->estado_propiedad == 'Retirada') {

				$ficha_propiedad = $obj->id_propiedad;
			} else {
			...
			\"$ficha_propiedad\",
			\"$obj->nombre_sucursal\",
			\"$obj->ejecutivo_encargado\",
			\"$propietarios_con_saltos\",
			\"$obj->tipo_propiedad\",
			\"$obj->direccion_concat\",
			\"$obj->comuna\",
			\"$obj->region\",
		...
		foreach ($json as $obj) {
			...
			if ($precio[0]->id_moneda_precio == 2) { // es peso
				...
				$montoFomateado = 'UF ' . $precio->precio;
			} else {
				$montoFomateado = $precio[0]->precio;
			}
			...
			$propietarios_con_saltos = str_replace("xxx", "<i class='fa-solid fa-house-user' style='color:#515151;font-size:12px;' title='Propietario' ></i> ", $propietarios);
			$propietarios_con_saltos = str_replace("zzz", "&nbsp;&nbsp;&nbsp;&nbsp;<i class='fa-solid fa-user-large' style='font-size:10px; color:#707070;' title='Beneficiario'></i> ", $propietarios_con_saltos);
...
$cadenaJSON = json_encode($dataTablePropiedades);
