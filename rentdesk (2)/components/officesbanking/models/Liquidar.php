...
function GuardarLiquidacion($services, $url_services, $id_ficha_propiedad, $id_propietario, $monto, $fecha_liquidacion, $id_ficha_arriendo, $url_liquidacion, $comision, $iva, $abonos, $descuentos, $total, $estado, $cierre)
{

    $query = "INSERT INTO propiedades.propiedad_liquidaciones(id_ficha_propiedad, id_propietario, monto, fecha_liquidacion, id_ficha_arriendo, url_liquidacion, comision, iva, abonos, descuentos, total, estado, cierre)VALUES ($id_ficha_propiedad, $id_propietario, $monto, '$fecha_liquidacion', $id_ficha_arriendo, '$url_liquidacion', $comision, $iva, $abonos,$descuentos, $total, $estado, $cierre)";
    $data = array("consulta" => $query);
    $resultado = $services->sendPostDirecto($url_services . '/util/dml', $data);
    return json_decode($resultado);
}

function GuardarDetalleLiquidacion($services, $url_services, $id_propiedad, $tipo_comision, $monto, $iva)
{

    $query = "INSERT INTO propiedades.propiedad_comision_liquidacion(id_propiedad_liquidacion, tipo_comision, monto, iva)
	VALUES ($id_propiedad, '$tipo_comision', $monto, $iva)";
    $data = array("consulta" => $query);
    $resultado = $services->sendPostDirecto($url_services . '/util/dml', $data);
...
function GenerarCorrelativo($services, $url_services)
{
    ...
    try {
        ...
        $checkQuery = "SELECT propiedades.correlativo()";
...
function generarPDF(
    ...
    try {
        ...
        $Numeros = GenerarCorrelativo($services, $url_services);
        $NumeroCierre = $Numeros[0]['correlativo'];
...
function procesarLiquidacion($ficha_tecnica_propiedad, $services, $url_services, $QueryBuilder)
{
    ...
    try {
        ...
        foreach ($datosPropietarios  as $index => $propietario) {
            ...
            $nombreArchivo = $carpetaPDFs . date('YmdHsmm') . '.pdf';
...
$archivoGenerado = procesarLiquidacion($ficha_tecnica_propiedad, $services, $url_services, $QueryBuilder);
