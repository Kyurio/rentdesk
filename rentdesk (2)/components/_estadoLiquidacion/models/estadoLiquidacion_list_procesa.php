...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(descripcion) LIKE '%$busqueda%' 
					 OR cast(id_estado_liquidacion as varchar) LIKE '$busqueda' OR  cast(orden as varchar)  LIKE '$busqueda')
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_estado_liquidacion $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.estado_liquidacion $busqueda ";
...
$query= "SELECT * from arpis.estado_liquidacion $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=estadoLiquidacion&view=estadoLiquidacion&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
$eliminar = "<a href='javascript: deleteEstadoLiquidacion(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";

$datos = $datos ."
     $signo_coma
	 [
      \"$result->id_estado_liquidacion\",
      \"$result->descripcion\",
	  \"$result->orden\",
	  \"$result->activo\",
	  \"$ver\",
