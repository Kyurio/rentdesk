...
$fechasDocumento = [];
	...
	$query = "SELECT id FROM propiedades.propiedad fa  where token = '$tokenFichaArriendo' ";
    $cant_rows = $num_reg;
    $num_pagina = round($inicio / $cant_rows) + 1;
    $data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
    $resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
...
foreach ($_FILES as $clave => $archivo) {
    if (strpos($clave, 'archivo_') !== false) {
        // Aquí puedes acceder a la información del archivo
        $nombre_archivo = $archivo["name"];
        $tipo_archivo = $archivo["type"];
        $tamano_archivo = $archivo["size"];
        ...
        $indice = substr($clave, strlen('archivo_'));
    ...
    if (strpos($clave, 'archivo_') !== false) {
		...
		///if (isset($fechasDocumento[$indice])) { //Se comenta por que puede ser que no venga una fecha
            ...
            if ($nombre_archivo != null && $nombre_archivo != "") { //Solo insertar cuando se tiene algún archivo
				...
				$doc_ima_fisico =  $nombre_sin_extension."___-___propiedad_".$nombre_archivo_token;
				move_uploaded_file($archivo["tmp_name"], "../../../upload/propiedad/" . $doc_ima_fisico.".".$extension);
				//move_uploaded_file($_FILES["archivo"]["tmp_name"], "../../../upload/arriendo/" . $doc_ima_fisico.".".$tipo_archivo);
				
				
				if ($tokenFichaArriendo != null && $tokenFichaArriendo != "" ){
					$queryCabecera= " INSERT INTO propiedades.propiedad_archivos
                    (id, id_usuario, fecha_subida, ruta, componente, archivo, titulo, fecha_vencimiento, id_ficha_arriendo, estado,extension,token_agrupador,nombre_archivo,token_defecto)
                     VALUES(nextval('propiedades.propiedad_archivos_id_seq'::regclass),$objUsuarioId->id, '$fecha','$carpeta' ,'propiedad', '$doc_ima_fisico' , '$documentoTitulo', '$fechaDocumento', $objArriendoId->id,true,'$extension','$token_agrupador','$nombre_sin_extension','$tokenFichaArriendo') ";
		        var_dump($queryCabecera);
					...
					$queryCabecera= " INSERT INTO propiedades.propiedad_archivos
                    (id, id_usuario, fecha_subida, ruta, componente, archivo, titulo, fecha_vencimiento, estado,extension,token_agrupador,nombre_archivo,token_defecto)
                     VALUES(nextval('propiedades.propiedad_archivos_id_seq'::regclass),$objUsuarioId->id, '$fecha','$carpeta' ,'propiedad', '$doc_ima_fisico' , '$documentoTitulo', '$fechaDocumento',true,'$extension','$token_agrupador','$nombre_sin_extension','$token_propiedad_defecto') ";
		        var_dump($queryCabecera);
                      $dataCab = array("consulta" => $queryCabecera);
                      $resultadoCab = $services->sendPostDirecto($url_services . '/util/dml', $dataCab);
				}
...
if ($fis_arch!="") {
				...
				$doc_ima_fisico =  $nombre_sin_extension."___-___arriendo_".$nombre_archivo;
				move_uploaded_file($_FILES["archivo"]["tmp_name"], "../../../upload/arriendo/" . $doc_ima_fisico.".". pathinfo($fis_arch, PATHINFO_EXTENSION));
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$queryCabecera= " INSERT INTO propiedades.propiedad_archivos
                    (id, id_usuario, fecha_subida, ruta, componente, archivo, titulo, fecha_vencimiento, id_ficha_arriendo, estado,extension)
                     VALUES(nextval('propiedades.propiedad_archivos_id_seq'::regclass),$objUsuarioId->id, '$fecha','$carpeta' ,'arriendo', '$doc_ima_fisico' , '$documentoTitulo', '$documentoFecha', $objArriendoId->id,true,'$extension') ";
		var_dump($queryCabecera);
              $dataCab = array("consulta" => $queryCabecera);
              $resultadoCab = $services->sendPostDirecto($url_services . '/util/dml', $dataCab);	
