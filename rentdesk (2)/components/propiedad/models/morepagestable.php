...
function Footer() {
	...
	if(!isset($this->footerset[$this->page])) {
		...
		$this->Cell(0,10,'         Minuta realizada con software minutario.cl by Emasmas',0,0,'C');
		$y = $this->GetY();
		$this->Image("../../../images/logo-168-70.png", 71 ,$y+2.3, 12.7);
		// set footerset
		$this->footerset[$this->page] = true;
	}
}

function morepagestable($datas, $lineheight, $align) {
	...
	foreach($datas AS $row => $data) {
		...
		foreach($data AS $col => $txt) {
			$this->page = $currpage;
			$this->SetXY($l,$h);
			$this->MultiCell($this->tablewidths[$col],$lineheight,$txt,'',$align);
