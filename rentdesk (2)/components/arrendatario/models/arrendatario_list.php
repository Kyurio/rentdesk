...
// $query_count = "SELECT * FROM propiedades.vis_arrendatarios";
...
// //var_dump("CANTIDAD ARRENDATARIOS: ", json_decode($cantidad_registros));

// $num_reg = 10;
// $inicio = 0;

// $query = "SELECT * FROM propiedades.vis_arrendatarios";
// $cant_rows = $num_reg;
// $num_pagina = round($inicio / $cant_rows) + 1;
// $data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
// $resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
...
/*BUSQUEDA CON FILTRO LISTA ARRENDATARIOS 
...
if ($json !== null) {
	...
	foreach ($json as $obj) {
		...
		$transformedObj = [
			...
			'token_arrendatario' => validateNull($obj->token_arrendatario ?? null),
		...
		$dataTableArrendatarios[] = $transformedObj;
	}
}
...
function redirectToArrendatarioUrl($token)
{
	...
	$redirectURL = "index.php?component=arrendatario&view=arrendatario" . $addTokenQuery;
...
function redirectToCreatePropiedad()
{
	...
	$redirectURL = "index.php?component=arrendatario&view=arrendatario";
