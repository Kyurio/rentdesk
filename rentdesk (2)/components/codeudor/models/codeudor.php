...
if (isset($nav)) {
	...
	$nav = "index.php?component=codeudor&view=codeudor_list";
}
//************************************************************************************************************
...
if (isset($token) && $token !== '') {
	...
	$query = "SELECT * FROM propiedades.vis_codeudores where token_codeudor = '$token'";
	$cant_rows = $num_reg;
	$num_pagina = round($inicio / $cant_rows) + 1;
	$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
	$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
	@$resultCodeudor = json_decode($resultado)[0];

	// //var_dump("PERSONA POR TOKEN: ", $result);
}

if (isset($resultCodeudor->token_persona)) {

	/*BUSQUEDA PERSONA POR TOKEN EN PROPIETARIO */
	$queryParams = array(
		'token_subsidiaria' => $current_subsidiaria->token,
		"token_persona" => $resultCodeudor->token_persona
	);
...
$queryParams = array(
	'token_codeudor' => $current_subsidiaria->token
);
...
foreach ($json as $item) {
	...
	if (@$resultCodeudor->token_persona == @$item->token) {
