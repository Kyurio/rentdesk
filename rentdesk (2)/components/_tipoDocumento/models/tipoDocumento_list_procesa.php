...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(descripcion) LIKE '%$busqueda%' 
					 OR cast(id_tipo_documento as varchar) LIKE '$busqueda' OR  cast(orden as varchar)  LIKE '$busqueda')
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_tipo_documento $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.tipo_documento $busqueda ";
...
$query= "SELECT * from arpis.tipo_documento $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=tipoDocumento&view=tipoDocumento&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
$eliminar = "<a href='javascript: deleteTipo_documento(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";
