...
$query = " SELECT tca.nombre, tca.id FROM  propiedades.tp_contrato_arriendo tca where habilitado = true and id in (1,2)  order by tca.nombre asc ";
$cant_rows = $num_reg;
...
$query = " SELECT p.nombre, p.id FROM  propiedades.tp_estado_propiedad p where habilitado = true order by p.nombre asc ";
$cant_rows = $num_reg;
$num_pagina = round($inicio / $cant_rows) + 1;
$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
$resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
...
// var_dump("dniPropietario ",$dniPropietario);
// var_dump("dniArrendatario ",);
