...
$orderby = " ORDER BY p.num_documento asc";
switch ($orden) {
	case 0:
...
$query_count = "SELECT * 
				...
				AND NOT EXISTS(SELECT 1 
							  FROM arpis.propiedad_propietario pp,
								   arpis.propiedad pro				
							   WHERE pro.token = '$token_propiedad'
							   AND pp.id_propiedad = pro.id_propiedad
							   AND pp.id_propietario = p.id_persona)
				$busqueda ";
...
$asignar = "";
$asignar = "<a href='javascript: agregarPropietario(\\\"$result->token\\\",\\\"$token_propiedad\\\",\\\"$participacion\\\");'><i class='fas fa-plus-square'></i></a>"; 
