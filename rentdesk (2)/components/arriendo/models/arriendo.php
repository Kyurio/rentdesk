...
$query = "SELECT  upper(nombre_1 || ' '  || nombre_2  || ' ' ||  nombre_3 ) as nombre_1 ,token_arrendatario FROM propiedades.vis_arrendatarios where token_subsidiaria = '$current_subsidiaria->token' ";
...
foreach ($json as $item) {
	...
	$opcion_arrendatario = $opcion_arrendatario . "<option value='$item->token_arrendatario' data-select2-id='$item->token_arrendatario' $select_arrendatario >$item->nombre_1</option>";
}

//$opcion_arrendatario = "<select id='arrendatario' name='arrendatario[]' class='form-control js-example-responsive' data-select2-id='arrendatarios'  multiple='multiple' required >
$opcion_arrendatario = "<select id='arrendatario' name='arrendatario[]' class='form-control js-example-responsive' data-select2-id='arrendatarios'  multiple='multiple' required >
$opcion_arrendatario
</select>";
...
$query = " SELECT nombre_1,token_codeudor FROM propiedades.vis_codeudores where token_subsidiaria = '$current_subsidiaria->token' ";
$data = array("consulta" => $query);
$resultado = $services->sendPostNoToken($url_services . '/util/objeto', $data);
@$json = json_decode($resultado);
...
$opcion_codeudor = "<option value=''>Seleccione</option>";
...
$opcion_codeudor = "<select id='codeudor' name='codeudor' class='form-control js-example-responsive' data-select2-id='codeudor' required >
$opcion_codeudor
</select>";
...
$query = " SELECT direccion ,token  FROM propiedades.propiedad where id_sucursal = $json_sucursal->id and id_subsidiaria  = $current_subsidiaria->id ";
$cant_rows = $num_reg;
$num_pagina = round($inicio / $cant_rows) + 1;
$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
...
$opcion_propiedad = "<option value=''>Seleccione</option>";
...
foreach ($json as $item) {
	...
	$opcion_propiedad = $opcion_propiedad . "<option value='$item->token' data-select2-id='$item->token' $select_propiedad >$item->direccion</option>";
}

$opcion_propiedad = "<select id='propiedad' name='propiedad' class='form-control js-example-responsive' data-select2-id='propiedad' required >
$opcion_propiedad
</select>";
