...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(descripcion) LIKE '%$busqueda%' 
					 OR cast(id_tipo_medio_pago as varchar) LIKE '$busqueda' OR  cast(orden as varchar)  LIKE '$busqueda')
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_tipo_medio_pago $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.tipo_medio_pago $busqueda ";
...
$query= "SELECT * from arpis.tipo_medio_pago $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=tipoMedioPago&view=tipoMedioPago&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
$eliminar = "<a href='javascript: deleteTipo_mediopago(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";

$datos = $datos ."
     $signo_coma
	 [
      \"$result->id_tipo_medio_pago\",
      \"$result->descripcion\",
	  \"$result->orden\",
	  \"$result->activo\",
	  \"$ver\",
