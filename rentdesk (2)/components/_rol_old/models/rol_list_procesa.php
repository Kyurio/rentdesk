...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(nombre) LIKE '%$busqueda%'
					 OR cast(id_rol as varchar) LIKE '$busqueda' )
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_rol $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.rol $busqueda ";
...
$query= "SELECT * from arpis.rol $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=rol&view=rol&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
$eliminar = "<a href='javascript: deleteRol(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";
