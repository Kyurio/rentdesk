...
var formData = new FormData(document.getElementById("formulario"));
   ...
   $.ajax({
                url: "components/tipoPersona/models/insert_update.php",
                type: "post",
                dataType: "html",
                data: formData,
                cache: false,
             ...
             .done(function(res){
				...
				if (resultado == 'OK'){
					...
					document.location.href="index.php?component=tipoPersona&view=tipoPersona&token="+token;
					return false;
				}else{
...
$(document).ready(function() { 
  ...
  $('#tabla').DataTable( {
	   "order": [[ 2, "asc" ]],
        ...
        "ajax": {
			"url":"components/tipoPersona/models/tipoPersona_list_procesa.php",
		"type": "POST"},
		"language": {
...
//************************************************************************
function deleteTipo_persona(token){
	...
	onSubmit: function (result) {
		if(result) {
				  $.ajax({
					  type: 'POST',
					  url: "components/tipoPersona/models/delete.php",
					  data:  "token="+token,
