...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(descripcion) LIKE '%$busqueda%'
					 OR cast(id_pack_cab as varchar) LIKE '$busqueda' )
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_pack_cab $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.pack_cab $busqueda ";
...
$query= "SELECT * from arpis.pack_cab $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=pack&view=pack&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
$eliminar = "<a href='javascript: deletePack(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";
