...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(descripcion) LIKE '%$busqueda%' 
					 OR cast(id_tipo_producto as varchar) LIKE '$busqueda' OR  cast(orden as varchar)  LIKE '$busqueda')
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_tipo_producto $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.tipo_producto $busqueda ";
...
$query= "SELECT * from arpis.tipo_producto $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=tipoProducto&view=tipoProducto&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
if(@$result->reservado == 'N'){
$eliminar = "<a href='javascript: deleteTipo_producto(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";
}

$datos = $datos ."
     $signo_coma
	 [
      \"$result->id_tipo_producto\",
      \"$result->descripcion\",
	  \"$result->orden\",
	  \"$result->activo\",
	  \"$ver\",
