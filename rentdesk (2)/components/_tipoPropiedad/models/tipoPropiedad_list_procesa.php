...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(descripcion) LIKE '%$busqueda%' 
					 OR cast(id_tipo_propiedad as varchar) LIKE '$busqueda' OR  cast(orden as varchar)  LIKE '$busqueda')
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_tipo_propiedad $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.tipo_propiedad $busqueda ";
...
$query= "SELECT * from arpis.tipo_propiedad $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=tipoPropiedad&view=tipoPropiedad&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
$eliminar = "<a href='javascript: deleteTipo_propiedad(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";

$datos = $datos ."
     $signo_coma
	 [
      \"$result->id_tipo_propiedad\",
      \"$result->descripcion\",
	  \"$result->orden\",
	  \"$result->activo\",
	  \"$ver\",
