...
$año_actual = date("Y");
$fecha_inicio = date("Y-m-d", strtotime("$año_actual-01-01"));
$fecha_termino=  date("Y-m-d", strtotime("$año_actual-12-31"));
...
foreach ($objMovimientos as $movimiento) {
	$fechaFormateada = @date('m-Y', strtotime($movimiento->mes));
    $meses[] = @$fechaFormateada;
