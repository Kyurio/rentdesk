...
$busqueda = " WHERE (arpis.fn_filtro_busqueda(descripcion) LIKE '%$busqueda%' 
					 OR cast(id_tipo_menu as varchar) LIKE '$busqueda' OR  cast(orden as varchar)  LIKE '$busqueda')
...
switch ($orden) {
	case 0:
		$orderby = " ORDER BY id_tipo_menu $direccion ";
		break;
	case 1:
...
$query_count = "SELECT * from arpis.tipo_menu $busqueda ";
...
$query= "SELECT * from arpis.tipo_menu $busqueda $orderby ";
$cant_rows = $num_reg;
...
$ver = "";
$ver = "<a href='index.php?component=tipoMenu&view=tipoMenu&token=$result->token'><i class='fas fa-search'></i></a>";

$eliminar = "";
$eliminar = "<a href='javascript: deleteTipo_menu(\\\"$result->token\\\");'><i class='far fa-trash-alt'></i></a>";
