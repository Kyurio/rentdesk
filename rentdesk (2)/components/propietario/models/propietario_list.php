...
@include("../models/busca_lista_filtros.php");
...
/*BUSQUEDA CON FILTRO LISTA PROPIETARIOS - DNI
...
$query = "SELECT vp.*
FROM propiedades.vis_propietarios vp
WHERE vp.token_subsidiaria in (
...
// $query_count = "SELECT * FROM propiedades.vis_propietarios";
...
// //var_dump("CANTIDAD PROPIETARIOS: ", json_decode($cantidad_registros));

// $num_reg = 10;
// $inicio = 0;

// $query = "SELECT * FROM propiedades.vis_propietarios";
// $cant_rows = $num_reg;
// $num_pagina = round($inicio / $cant_rows) + 1;
// $data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
// $resultado = $services->sendPostNoToken($url_services . '/util/paginacion', $data, []);
...
function redirectToPropietarioUrl($token)
{
	...
	$redirectURL = "index.php?component=propietario&view=propietario" . $addTokenQuery;
