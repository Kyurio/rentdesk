...
$query_count = "SELECT * 
				FROM arpis.propiedad p,
					 ...
					 arpis.persona a,
					 arpis.propiedad_propietario pp
				...
				AND pp.id_propiedad = cc.id_propiedad	
				AND a.id_persona = pp.id_propietario
				AND a.token = '$token'
...
$query= "SELECT p.token,tp.descripcion tipo_propiedad,p.codigo_propiedad,p.rol,p.direccion,p.numero,p.numero_depto,ep.descripcion  estado_propiedad,cc.token token_contrato,p.token token_propiedad,
                (SELECT lc2.token FROM arpis.liquidacion_cab lc2 WHERE lc2.id_liquidacion_cab =(SELECT max(id_liquidacion_cab) FROM arpis.liquidacion_cab lc where lc.id_contrato = cc.id_contrato)) liquidacion
		FROM arpis.propiedad p,
			 arpis.tipo_propiedad tp,
			 arpis.estado_propiedad ep,
			 arpis.contrato_cab cc,
...
$ver = "";
$ver = "<a href='index.php?component=liquidacion&view=liquidacion&token=$result->liquidacion&nav=$nav''><i class='fas fa-search'></i></a>";

$hist = "";
$hist = "<a href='index.php?component=liquidacion&view=liquidacion_list&token_contrato=$result->token_contrato&token=$token&nav=$nav''><i class='fas fa-history'></i></a>";
